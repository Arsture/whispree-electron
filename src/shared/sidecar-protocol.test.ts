...
describe('sidecar protocol', () => {
  it('validates health, transcription, correction, vision, cancel, progress, and error envelopes', () => {
    ...
    expect(isSidecarRequest({ id: '3', protocolVersion: 1, type: 'correct', text: 'hello', mode: 'standard', glossary: [], systemPrompt: 'Swift prompt' })).toBe(true);
    expect(isSidecarRequest({ id: '4', protocolVersion: 1, type: 'vision-correct', text: 'hello', imageRefs: ['shot'], mode: 'standard', glossary: [], systemPrompt: 'Swift prompt' })).toBe(true);
