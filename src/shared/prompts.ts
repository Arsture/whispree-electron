import type { CorrectionMapping, CorrectionMode, SupportedLanguage } from './settings';
...
export const correctionPrompts = {
  defaultSystemPrompt: `You are an STT post-processor. Fix ONLY clear speech-to-text errors.
...
Output ONLY the corrected text. No explanations.`,
  ...
  englishOnlyPrompt: `You are an STT post-processor. Fix ONLY clear speech-to-text errors.
...
Output ONLY the corrected text. No explanations.`,
  ...
  koreanOnlyPrompt: `당신은 한국어 음성인식 후처리 전문가입니다.
...
교정된 텍스트만 출력하세요.`,
  ...
  codeSwitchPrompt: `당신은 한국어-영어 코드스위칭 음성인식 후처리 전문가입니다.
...
교정된 텍스트만 출력하세요. 설명은 하지 마세요.`,
  ...
  fillerRemovalPrompt: `당신은 한국어-영어 코드스위칭 음성인식 후처리 전문가입니다.
...
교정된 텍스트만 출력하세요. 설명은 하지 마세요.`,
  ...
  structuredPrompt: `당신은 한국어-영어 코드스위칭 음성인식 후처리 및 구조화 전문가입니다.
...
교정된 텍스트만 출력하세요. 설명은 하지 마세요.`,
  ...
  screenshotContextPrompt: `

[시각 맥락]
첨부된 스크린샷들은 사용자가 녹음하는 동안 보고 있던 화면들입니다 (시간 순서대로 배열).
여러 애플리케이션을 오가며 작업하는 흐름일 수 있습니다.
각 화면을 분석하여 사용 중인 도구, 보이는 기술 용어, 다루는 주제를 종합적으로 파악하세요.
...
export function correctionPromptForMode(mode: CorrectionMode, language: SupportedLanguage): string {
  if (mode === 'filler-removal') return correctionPrompts.fillerRemovalPrompt;
  if (mode === 'structured') return correctionPrompts.structuredPrompt;
  if (mode === 'custom') return correctionPrompts.codeSwitchPrompt;
  if (language === 'ko') return correctionPrompts.koreanOnlyPrompt;
  if (language === 'en') return correctionPrompts.englishOnlyPrompt;
  if (language === 'auto') return correctionPrompts.codeSwitchPrompt;
  return correctionPrompts.defaultSystemPrompt;
}
...
export function buildCorrectionSystemPrompt({
  ...
  readonly includeScreenshotPrompt: boolean;
}): string {
  let systemPrompt = mode === 'custom'
    ? customPrompt?.trim() || correctionPrompts.codeSwitchPrompt
    : correctionPromptForMode(mode, language);

  if (correctionMappings.length > 0) {
    const mappingText = correctionMappings.map((mapping) => `${mapping.from} → ${mapping.to}`).join('\n');
    systemPrompt += `\n\n교정 매핑 (왼쪽 표현이 텍스트에 있으면 오른쪽으로 교정):\n${mappingText}`;
  }

  if (includeScreenshotPrompt) systemPrompt += correctionPrompts.screenshotContextPrompt;
  return systemPrompt;
}
